...
export function QueueMetricStrip({
  ...
  return (
    <section className="grid gap-3 xl:grid-cols-5">
      {items.map((item) => (
        <Link key={item.label} href={item.href} className="panel flex min-h-[104px] flex-col justify-between p-4 transition hover:-translate-y-0.5">
          ...
          <div className="flex items-end justify-between gap-3">
            ...
            <ArrowRight className="rtl-flip size-4 text-[var(--color-accent)]" />
          </div>
        </Link>
      ))}
    </section>
