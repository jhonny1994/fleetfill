...
begin
  ...
  if v_source_type = 'route' then
    ...
    v_total_capacity_kg := coalesce(v_route_instance.total_capacity_kg, v_route_revision.total_capacity_kg);
  ...
  values (
    ...
    v_payment_reference,
    case
      when v_request_key is null then '{}'::jsonb
      else jsonb_build_object('idempotency_key', v_request_key)
    end
  )
  ...
  select
    'booking_confirmed',
    'booking_confirmed:' || coalesce(v_request_key, v_booking.id::text),
    p.id,
    v_booking.id,
    'booking_confirmed',
    'en',
