...
async function fetchBookingListPage({
  page = 1,
  pageSize = 25,
  query,
  status,
  ...
  let countRequest = supabase.from("bookings").select("id", { count: "exact", head: true });
  ...
  if (normalizedStatus && isBookingIndexStatus(normalizedStatus)) {
    countRequest = countRequest.eq("booking_status", normalizedStatus as never);
    pageRequest = pageRequest.eq("booking_status", normalizedStatus as never);
  }

  const normalizedQuery = query?.trim();
  if (normalizedQuery) {
    const filter = buildBookingSearchFilter(normalizedQuery);
    countRequest = countRequest.or(filter);
    pageRequest = pageRequest.or(filter);
  }

  const countResult = await countRequest;
  if (countResult.error) throw countResult.error;
  const { page: safePage } = paginate(countResult.count ?? 0, page, pageSize);
  const { from, to } = pageRange(safePage, pageSize);
  const pageResult = await pageRequest.range(from, to);
  if (pageResult.error) throw pageResult.error;

  const bookingRows = (pageResult.data ?? []) as BookingIndexRow[];
  if (bookingRows.length === 0) {
    return {
      ...
      page: safePage,
      pageSize,
      totalPages: paginate(countResult.count ?? 0, safePage, pageSize).totalPages,
    };
  }

  const [profilesResult, proofsResult, disputesResult] = await Promise.all([
    supabase
      .from("profiles")
      .select("id, full_name, email, company_name")
      .in(
        "id",
        [...new Set(bookingRows.flatMap((row) => [row.shipper_id, row.carrier_id]).filter(Boolean))] as string[],
      ),
    supabase.from("payment_proofs").select("booking_id").in("booking_id", bookingRows.map((row) => row.id)),
    supabase.from("disputes").select("booking_id, status").in("booking_id", bookingRows.map((row) => row.id)),
  ]);
  ...
  const profileMap = new Map(
    ((profilesResult.data ?? []) as ProfileRow[]).map((profile) => [profile.id, resolveDisplayName(profile)]),
  );
  const proofCounts = new Map<string, number>();
  for (const proof of (proofsResult.data ?? []) as Array<{ booking_id: string }>) {
    proofCounts.set(proof.booking_id, (proofCounts.get(proof.booking_id) ?? 0) + 1);
  }
  const disputeMap = new Map(
    ((disputesResult.data ?? []) as Array<{ booking_id: string; status: string }>).map((dispute) => [
      dispute.booking_id,
      dispute.status,
    ]),
  );
  ...
  return {
    items: bookingRows.map((row) => ({
      ...
      shipperDisplayName: profileMap.get(row.shipper_id) ?? row.shipper_id,
      carrierDisplayName: row.carrier_id ? profileMap.get(row.carrier_id) ?? row.carrier_id : null,
      shipperName: profileMap.get(row.shipper_id) ?? row.shipper_id,
      carrierName: row.carrier_id ? profileMap.get(row.carrier_id) ?? row.carrier_id : null,
      paymentReference: row.payment_reference,
      shipmentDescription: null,
      originLabel: row.shipment_id,
      destinationLabel: row.shipment_id,
      shipperTotalDzd: Number(row.shipper_total_dzd),
      carrierPayoutDzd: Number(row.carrier_payout_dzd),
      paymentProofCount: proofCounts.get(row.id) ?? 0,
...
async function fetchShipmentListPage({
  page = 1,
  pageSize = 25,
  query,
  status,
  ...
  await requireServerAdminSession();
  const supabase = await createSupabaseServerClient();
  let countRequest = supabase.from("shipments").select("id", { count: "exact", head: true });
  let pageRequest = supabase
    .from("shipments")
  ...
  if (normalizedStatus && isShipmentIndexStatus(normalizedStatus)) {
    countRequest = countRequest.eq("status", normalizedStatus as never);
    pageRequest = pageRequest.eq("status", normalizedStatus as never);
  }

  const normalizedQuery = query?.trim();
  if (normalizedQuery) {
    const filter = buildShipmentSearchFilter(normalizedQuery);
    countRequest = countRequest.or(filter);
    pageRequest = pageRequest.or(filter);
  }
  ...
  const shipmentRows = (pageResult.data ?? []) as ShipmentIndexRow[];
  ...
  const communeIds = [
    ...new Set(shipmentRows.flatMap((row) => [row.origin_commune_id, row.destination_commune_id])),
  ];
  const [profilesResult, bookingsResult, communesResult] = await Promise.all([
    supabase
      .from("profiles")
      .select("id, full_name, email, company_name")
      .in("id", [...new Set(shipmentRows.map((row) => row.shipper_id))]),
    supabase
      .from("bookings")
      .select("id, shipment_id, tracking_number, booking_status, payment_status, created_at")
      .in("shipment_id", shipmentRows.map((row) => row.id)),
  ...
  const bookingMap = new Map(
    ((bookingsResult.data ?? []) as ShipmentBookingRow[]).map((booking) => [booking.shipment_id, booking]),
  );
  ...
  return {
    items: shipmentRows.map((row) => ({
      ...
      shipperDisplayName: profileMap.get(row.shipper_id) ?? row.shipper_id,
      shipperName: profileMap.get(row.shipper_id) ?? row.shipper_id,
      bookingId: bookingMap.get(row.id)?.id ?? null,
      bookingTrackingNumber: bookingMap.get(row.id)?.tracking_number ?? null,
      bookingStatus: bookingMap.get(row.id)?.booking_status ?? null,
      paymentStatus: bookingMap.get(row.id)?.payment_status ?? null,
