...
- [x] Make scheduler wiring and timed-automation invocation more explicit in repo-managed infrastructure so durable automation does not depend on undocumented operator setup (`supabase/scripts/configure_scheduled_automation.sql`, `supabase/README.md`, `supabase/functions/scheduled-automation-tick/index.ts`, `docs/03-technical-architecture.md`). This path now uses a dedicated internal automation token rather than reusing the raw service-role bearer for worker endpoint auth.
